...
% Elaine Zaunseder - 2023 (adapted for infants)

fileNameOrgan = 'Newborn_OrganWeights.xlsx';
...
Col=2;
organRow=4;
OrganWeightFract = Numbers(2:end-2,Col)/100; % assumes that the relative organ weights are given in %
OrganWeight = Numbers(2:end-2,Col-1); %for newborns 
%OrganWeight = Numbers(2:end,Col-1); % assumes that the organ weights appear in the col before the percentage
OrganNames = organWeightData(organRow:end-2,1);
